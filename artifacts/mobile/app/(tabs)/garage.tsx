...
export default function GarageScreen() {
  ...
  const activeOverviewVehicle = vehicles[clampedSlideIndex] ?? vehicles[0];
  // Personal vehicles intentionally use the same completed-session dataset as
  // Trip History so both summaries always agree. Shared vehicles retain the
  // privacy-preserving server aggregate across all co-drivers.
  const useSharedTotals = Boolean(activeOverviewVehicle?.sharedVehicleId && sharedStats);
  const totalDistKm = useSharedTotals
    ? sharedStats!.totalDistM / 1000
    : completed.reduce((a, s) => a + s.distanceM, 0) / 1000;
  const totalDurS = useSharedTotals
    ? sharedStats!.totalDurS
    : completed.reduce((a, s) => a + (s.durationS ?? 0), 0);
  const totalTrips = useSharedTotals
