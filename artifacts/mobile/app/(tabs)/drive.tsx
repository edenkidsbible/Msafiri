...
export default function DriveScreen() {
  ...
  const { isSubscribed, isOnTrial, trialExpiredUnpaid } = useSubscription();
  const {
      ...
      if (deviceId) {
        ...
        startDriveSession(deviceId, currentLat, currentLng, drivingVehicleId, drivingSharedVehicleId)
          .then((id) => {
            ...
            if (drivingVehicleId) recordSession(drivingVehicleId, id).catch(() => {});
          })
  ...
  useEffect(() => {
    ...
    AsyncStorage.getItem(FREE_DRIVE_STARTED_AT_KEY)
      .then((persisted) => {
        ...
        timeout = setTimeout(() => {
          if (!tripActive || (isSubscribed && !isOnTrial) || trialExpired) return;
  ...
  }, [tripActive, trialLoading, isSubscribed, isOnTrial, trialExpired]);
  ...
  }, [tripActive, isSubscribed, isOnTrial, trialExpired]);
