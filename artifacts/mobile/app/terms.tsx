...
export default function TermsScreen() {
  ...
  return (
    ...
    >
      ...
      <Li>{"Send repeated vehicle join requests or plate searches to harass vehicle owners or overwhelm our servers. Our API enforces rate limits — repeated attempts after a 429 response are a violation of these Terms."}</Li>
      ...
      <Li>{"Share, reproduce, or distribute dashcam footage of other people without their consent, in violation of Kenyan privacy law or any other applicable law."}</Li>
      ...
      <Li>{"Route data is calculated by the Google Maps Routes API. Routes may be incorrect, outdated, or unavailable in areas with limited map coverage."}</Li>
      ...
      <Text style={[s.p, { color: c.mutedForeground }]}>
        ...
        by rotating IP addresses to exceed the per-IP limit) is a violation of these Terms and may result
        in permanent access restriction.
      </Text>
      ...
      <Li><B>Your recordings, your responsibility.</B>{" You are solely responsible for ensuring that your use of the dashcam complies with all applicable Kenyan laws and any laws applicable in your jurisdiction, including laws regarding the recording of individuals in public or private spaces."}</Li>
      <Li>{"Msafiri does not access, view, process, or analyse your dashcam footage unless you explicitly share a clip with us for support purposes."}</Li>
      <Li>{"If you enable cloud backup, locked clips are uploaded to our secure storage over Wi-Fi. You may delete cloud-backed clips at any time from within the App."}</Li>
      <Li>{"We are not responsible for the loss of any recording due to device storage limitations, app crashes, or other technical failures. We recommend locking important clips promptly after an incident."}</Li>
      ...
      <View style={[s.warning, { backgroundColor: c.primary + "14", borderColor: c.primary + "4D" }]}>
        <Text style={[s.p, { color: c.foreground, marginBottom: 0 }]}>
          <B>The Accident Assistant is a documentation aid only.</B>{" It does not provide legal, insurance, or medical advice. Always call emergency services (999 in Kenya) if anyone is injured. Always consult a qualified legal professional before making any formal legal claim."}
        </Text>
      </View>
      <Li>{"Accuracy of the documentation generated by the Accident Assistant depends entirely on the information you enter. Msafiri Kenya is not responsible for inaccuracies in incident reports arising from incorrect user input."}</Li>
      <Li>{"The PDF accident report generated by the App is produced for your personal use only. Msafiri Kenya does not transmit this report to any insurer, legal authority, or third party on your behalf."}</Li>
      <Li>{"Scene photographs uploaded through the Accident Assistant are stored on our secure servers. You may request deletion of these files at any time by contacting privacy@msafirikenya.com."}</Li>
      ...
      <Text style={[s.p, { color: c.mutedForeground }]}>
        ...
        recovery code and your vehicle plate number.
      </Text>
      <Li><B>Keep your recovery code safe.</B>{" Msafiri Kenya cannot retrieve a lost recovery code. If both your code and your device are lost, your backup data cannot be restored."}</Li>
      <Li>{"You are responsible for entering correct information during a restore. If incorrect credentials are entered, the restore will be refused. Repeated failed restore attempts may trigger a temporary restriction on your IP address."}</Li>
      <Li>{"Backup data does not include dashcam footage, crash assistant photos, or location history — only app configuration and vehicle data."}</Li>
      ...
      <Li>{"Loss or corruption of dashcam footage or crash assistant documentation."}</Li>
      <Li>{"Outcomes of insurance claims or legal proceedings in which App-generated data is used."}</Li>
      ...
      <Li>{"Any dashcam footage you record, store, share, or use in a manner that violates any law or third-party rights."}</Li>
      <Li>{"Unauthorised use or disclosure of co-driver share codes."}</Li>
      ...
      <Li>{"You remain bound by any provisions of these Terms that by their nature should survive termination (including Sections 16, 17, 18, 20, and 21)."}</Li>
      ...
      <Li><B>RevenueCat</B>{" — subscription management."}</Li>
      <Li><B>Google Maps SDK & Routes API</B>{" — map rendering and turn-by-turn navigation routing."}</Li>
      <Li><B>ElevenLabs</B>{" — on-demand voice guidance text-to-speech."}</Li>
      <Li><B>OpenStreetMap / Nominatim / Photon / Overpass API</B>{" — geocoding, road name lookup, and points-of-interest search."}</Li>
      <Li><B>SMSLeopard</B>{" — SMS delivery for one-time passcode (OTP) verification."}</Li>
      <Li><B>Cloudflare R2</B>{" — secure cloud storage for dashcam backups and crash assistant photos."}</Li>
      <Text style={[s.p, { color: c.mutedForeground }]}>
        Your use of these third-party services is governed by their own terms and conditions. Msafiri
        Kenya is not responsible for the acts or omissions of any third-party service provider.
      </Text>
      <Text style={[s.p, { color: c.mutedForeground }]}>
