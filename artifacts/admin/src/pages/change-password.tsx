...
import { useAdminChangePassword, useAdminGetMe } from "@workspace/api-client-react";
...
export default function ChangePassword() {
  ...
  // Fetch current 2FA status so TotpSetup can render correctly.
  const { data: meData, refetch: refetchMe } = useAdminGetMe({
    query: { queryKey: ["/api/admin/auth/me"], enabled: !!token },
  });
    ...
    <div className="min-h-screen bg-muted/30 flex flex-col items-center justify-center p-4 py-12">
      <div className="w-full max-w-[440px] space-y-6">
        ...
        <PageGuide
          ...
          steps={[
            { label: "Strong password", detail: "your new password must be at least 8 characters." },
            { label: "Enable 2FA", detail: "two-factor authentication protects your account even if your password is compromised — use an authenticator app like Google Authenticator." },
        ...
        {/* ── 2FA setup (only shown when not in forced-change mode) ── */}
        {!isForcedChange && (
          <>
            <Separator />
            <TotpSetup
              totpEnabled={meData?.totpEnabled ?? false}
              onChanged={() => refetchMe()}
            />
          </>
        )}
