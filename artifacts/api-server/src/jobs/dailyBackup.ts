...
export async function buildBackupSnapshot(): Promise<BackupSnapshot> {
  ...
  const [
    ...
    fetch("creator_benefits",         () => db.select().from(creatorBenefitsTable).orderBy(creatorBenefitsTable.createdAt)),
    fetch("creator_subscription_events", () => db.select().from(creatorSubscriptionEventsTable).orderBy(creatorSubscriptionEventsTable.createdAt)),
